...
require 'sampfuncs'
-- ============================================================================
-- 2. НАСТРОЙКИ АВТОРИЗАЦИИ И URL СЕРВЕРОВ
-- ============================================================================
-- ВАЖЛИВО: Вставте сюди ваше посилання на розгорнутий Google Apps Script Webhook
local WEBHOOK_URL  = "https://script.google.com/macros/s/AKfycbzxAKwMAL86-AFsVlZHN4DAAz6-VY-byX9MLEmG68n2PGoce5YsCGVinTZ8i27ex0yYnQ/exec"
...
has_access = false,
role       = "none",
reason     = "Очікування перевірки доступу...",
expire     = "-"
}
-- Отримання HWID через Windows API
ffi.cdef[[
...
-- ============================================================================
-- 6. ЗАГРУЗКА И ВАЛИДАЦИЯ НАСТРОЕК
-- ============================================================================
...
-- ============================================================================
-- 7. ФУНКЦИЯ ПРОВЕРКИ АВТОРИЗАЦИИ (GOOGLE SHEETS)
-- ============================================================================
function check_access()
user_auth.is_checked = false
user_auth.reason = "Перевірка доступу..."
lua_thread.create(function()
     while not isSampAvailable() do wait(100) end
     local _, my_id = sampGetPlayerIdByCharHandle(PLAYER_PED)
     local nick = sampGetPlayerNickname(my_id)
     local hwid = get_hwid()
     local post_data = encodeJson({
         nick = nick,
         hwid = hwid
     })
     ...
     -- Цикл для обробки 302/301 перенаправлень від Google
     repeat
         response_body = {}
         local req_headers = {
         ...
         -- Якщо це перенаправлення, надсилаємо GET або зберігаємо параметри
         local method = (current_redirect > 0) and "GET" or "POST"
         local source_val = (method == "POST") and ltn12.source.string(post_data) or nil
         if method == "GET" then req_headers["Content-Length"] = nil end
         local res
         res, code, headers, status = https.request({
             ...
             redirect = false -- вимикаємо стандартне оброблення, керуємо вручну
         })
     ...
     until current_redirect >= max_redirects
     local raw_res = table.concat(response_body)
     if code == 200 and raw_res and #raw_res > 0 then
         local ok, data = pcall(decodeJson, raw_res)
         if ok and type(data) == "table" then
             if data.access then
                 user_auth.has_access = true
                 user_auth.role       = tostring(data.role or "user"):lower()
                 
                 -- ПЕРЕВОД lifetime НА РУССКИЙ
                 local expire_raw = tostring(data.expire or "lifetime")
                 if expire_raw:lower() == "lifetime" then
                     user_auth.expire = "Бессрочно"
                 else
                     user_auth.expire = expire_raw
                 end
                 
                 user_auth.reason     = "Успішно авторизовано"
                 
                 -- ОБЪЕДИНЕННОЕ СООБЩЕНИЕ (СЕРЫЙ ЦВЕТ)
                 sampAddChatMessage(cyr(string.format("[DH] Скрипт загружен. Доступ разрешен. Роль: %s | Срок: %s", user_auth.role:upper(), user_auth.expire)), 0xffcccccc)
             else
                 user_auth.has_access = false
                 user_auth.reason     = data.reason or "Відмовлено у доступі"
                 
                 -- СООБЩЕНИЕ О БАНЕ/ОТКАЗЕ (ТЕМНО-КРАСНЫЙ ЦВЕТ)
                 sampAddChatMessage(cyr(string.format("[DH] Ошибка доступа: %s", user_auth.reason)), 0x8B0000)
             end
         else
             user_auth.has_access = false
             user_auth.reason     = "Некоректна відповідь сервера"
             sampAddChatMessage(cyr("[DH] Ошибка: Некорректный ответ сервера"), 0x8B0000)
         end
     else
         user_auth.has_access = false
         user_auth.reason     = "Помилка з'єднання з веб-сервером (Код: " .. tostring(code) .. ")"
         sampAddChatMessage(cyr("[DH] Ошибка соединения с сервером"), 0x8B0000)
     end
     user_auth.is_checked = true
 end)
end
-- ============================================================================
-- 8. АВТООБНОВЛЕНИЕ СКРИПТА И ЛЕНТА НОВОСТЕЙ
-- ============================================================================
...
-- ============================================================================
-- 9. АВТОПЕРЕПОДКЛЮЧЕНИЕ К СЕРВЕРУ
-- ============================================================================
...
-- ============================================================================
-- 10. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ ИНТЕРФЕЙСА (mimgui)
-- ============================================================================
...
-- ============================================================================
-- 11. СОСТОЯНИЕ И РАЗМЕТКА ОКНА MIMGUI
-- ============================================================================
...
-- Окно будет всегда выравниваться ровно по центру экрана ( pivot 0.5, 0.5 )
imgui.SetNextWindowPos(imgui.ImVec2(x / 2, y / 2), imgui.Cond.Always, imgui.ImVec2(0.5, 0.5))
-- Устанавливаем увеличенный размер (например, 1000x650) и флаг Always, чтобы он был фиксированным
 imgui.SetNextWindowSize(imgui.ImVec2(1000, 650), imgui.Cond.Always)
 
 -- Добавляем флаг NoResize, который запрещает изменять размер окна мышью
 ...
 imgui.Begin('DH Script', window, flags)
 -- --------------------------------------------------------------------
 -- ПРОВЕРКА АВТОРИЗАЦИИ
 -- --------------------------------------------------------------------
 if not user_auth.has_access then
     imgui.Spacing()
     imgui.TextColored(imgui.ImVec4(1.0, 0.2, 0.2, 1.0), 'ОШИБКА ДОСТУПА / АВТОРИЗАЦИИ')
     imgui.Separator()
     imgui.Spacing()
     imgui.TextWrapped(string.format("Статус: %s", user_auth.reason))
     imgui.Spacing()
     imgui.TextDisabled("Проверьте регистрацию вашего никнейма и HWID в базе данных.")
     imgui.Spacing()
     if not user_auth.is_checked then
         imgui.TextColored(imgui.ImVec4(1.0, 0.8, 0.2, 1.0), "Идет проверка авторизации, подождите...")
     else
         if imgui.Button('Повторить проверку доступа', imgui.ImVec2(-1, 30)) then
             check_access()
         end
     end
     imgui.End()
 ...
 -- ====================================================================
 -- ВЕРХНИЙ БЛОК: ПРОФИЛЬ ПОЛЬЗОВАТЕЛЯ (ИДЕАЛЬНО ПО ЦЕНТРУ И АДАПТИВНО)
 -- ====================================================================
 local _, my_id = sampGetPlayerIdByCharHandle(PLAYER_PED)
 local my_nick = sampGetPlayerNickname(my_id) or "Unknown"
 local header_height = 36
 imgui.BeginChild('##user_profile_header', imgui.ImVec2(-1, header_height), true, imgui.WindowFlags.NoScrollbar)
     -- Выравнивание по вертикальному центру
     local style = imgui.GetStyle()
     local text_height = imgui.GetTextLineHeight()
     local pad_y = (header_height - text_height - (style.WindowPadding.y * 2)) / 2
     imgui.SetCursorPosY(imgui.GetCursorPosY() + pad_y)
     local total_w = imgui.GetContentRegionAvail().x
     -- Игрок
     imgui.TextDisabled("Игрок:")
     imgui.SameLine()
     imgui.Text(my_nick)
     -- HWID
     imgui.SameLine(total_w * 0.32)
     imgui.TextDisabled("HWID:")
     imgui.SameLine()
     imgui.Text(get_hwid())
     -- Роль
     imgui.SameLine(total_w * 0.62)
     imgui.TextDisabled("Роль:")
     imgui.SameLine()
     local role_color = imgui.ImVec4(0.60, 0.60, 0.60, 1.00) -- user / gray
     if user_auth.role == "admin" then
         role_color = imgui.ImVec4(1.00, 0.25, 0.25, 1.00) -- red
     elseif user_auth.role == "vip" then
         role_color = imgui.ImVec4(1.00, 0.80, 0.00, 1.00) -- gold
     end
     imgui.TextColored(role_color, user_auth.role:upper())
     -- Срок
     imgui.SameLine(total_w * 0.82)
     imgui.TextDisabled("Срок:")
     imgui.SameLine()
     imgui.Text(user_auth.expire)
 imgui.EndChild()
 imgui.Spacing()
 -- ====================================================================
 -- ОСНОВНАЯ ЧАСТЬ (АДАПТИВНАЯ СЕТКА)
 -- ====================================================================
 ...
 -- Пропорциональное разделение (58% левая колонка, остаток — правая)
 ...
 if user_auth.role == "admin" or user_auth.role == "vip" then
     if imgui.BeginTabItem('VIP / Admin') then
         imgui.Spacing()
         imgui.TextColored(imgui.ImVec4(0.2, 0.8, 1.0, 1.0), "Панель VIP / Администратора")
         imgui.Separator()
         imgui.Spacing()
         imgui.TextDisabled("Здесь можно разместить эксклюзивные VIP функции.")
         imgui.EndTabItem()
     end
 end
 imgui.EndTabBar()
 ...
 local button_h = 28
 local style = imgui.GetStyle()
 -- Вычисляем точную высоту под список новостей с учетом отступов
 ...
 -- Отступ перед кнопкой и выравнивание внизу
 imgui.SetCursorPosY(main_content_height - button_h)
 if imgui.Button('Обновить ленту##feed_refresh', imgui.ImVec2(-1, button_h)) then
     check_updates_and_feed()
 end
 ...
 imgui.PopStyleColor()
 -- --------------------------------------------------------------------
 -- КНОПКА ОБНОВЛЕНИЯ СКРИПТА (ЕСЛИ ЕСТЬ НОВАЯ ВЕРСИЯ)
 -- --------------------------------------------------------------------
...
-- УДАЛЕНО СТАРОЕ СООБЩЕНИЕ О ЗАГРУЗКЕ
-- sampAddChatMessage(cyr('[DH] завантажено'), 0xffcccccc)

-- Автоматична перевірка доступу при старті
 check_access()
 check_updates_and_feed()
 ...
 while true do
     wait(0)
     -- Якщо немає доступу, основна функціональність скрипта вимикається
     if user_auth.has_access then
